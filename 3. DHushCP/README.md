...
```
   cd ~/dhushcp
   sudo .dhushcp/bin/python DHushCP/dhushcp/responder.py --id DHUSHCP_ID
```

**Initiator:**
```
   cd ~/dhushcp
   sudo .dhushcp/bin/python DHushCP/dhushcp/initiator.py --id DHUSHCP_ID
```
...
1. **Initial Exchange:**
   - **Initiator:**
     ...
     - *Here are the Initiator's three identifiers, using print() (see [DEBUG])*.
![i1](docs/i1.png)
     - Embeds the pre-shared DHUSHCP_ID (option 224), the session ID (option 225) and its public key (option 226) into the DHCP Discover packet.
     - Sends the DHCP Discover packet and waits for the Responder's public key.
![i2](docs/i2.png)
   
   - **Responder:**
     - Generates its own ECC key pair.
     - *Here's the Responder's public key, using print() (see [DEBUG])*.
![r1](docs/r1.png)
...
![r2](docs/r2.png)
     - Extracts and reassembles the Initiator's public ECC key from DHCP option 226.
...
![i4](docs/i4.png)
   ...
   - **Responder:**
     - Receives the DHCP Discover packet with the encrypted message from the Initiator.
     - Extracts and decrypts the message using the shared AES key it also derived.
![r4](docs/r4.png)
...
- There is no centralized app or device, **no visible Wi-Fi connection** or direct link between Alice and Bob.
- After the communication ends, both machines **securely delete** the ECC keys, system logs and clear the terminal.
...
- Prior to running the Initiator or the Responder, **disable shell history** using `set +o history` (Ubuntu), then re-enable it when the communication is ended using `set -o history`.
